...
public class SolveMethod : MonoBehaviour {
	...
	public static List<int> piecesturned = new List<int>();
	...
	public void Solve(string[,] tiles){	//single solution
		...
		CheckAndCreate(0, startx, starty, currenttiles, initialgenes, totalstoppedtiles, 0, new List<Vector2>(), 0, piecesturned);	//Creates first worker (Unless he can move more than one place)
...
//			Debug.Log("Cant");
	...
	public void CheckAndCreate(int newturns, int x, int y,string[,] thistiles, List<string> newgenes, List<Vector2> newstoppedtiles, 
		int newlrud, List<Vector2> newlrudpos, int newpiecesused, List<int> newpiecesturned){
		//workers.Clear();
		...
		lrud = newlrud;
		lrudpos = newlrudpos;
		int mymapdimension = CreateMethod.mapdimension;
		//List<int> mypiecesturned = newpiecesturned;

		//List<Map> mystoppedmaps = newstoppedmaps;
		//int mypiecesturned = newpiecesturned;
		//List<int>myturnedperturn = newturnedperturn;
		if(x<mymapdimension-1){
			if(thistiles[x+1,y] != "Wall"){//Checking Right
				if(turns == 0 && thistiles[x+1,y] == "Goal"){
				}
				else{
					//Debug.Log("Cloning Right at " + x + y);
					Worker worker = new Worker(turns, x, y, thistiles, "Right", newgenes, newstoppedtiles,lrud, lrudpos, newpiecesused, newpiecesturned);
					workers.Add(worker);
				}		
			}
		}
		...
		//			Debug.Log("Cloning Left");
					Worker worker = new Worker(turns, x, y, thistiles, "Left", newgenes, newstoppedtiles,  lrud, lrudpos, newpiecesused, newpiecesturned);
					workers.Add(worker);
				}
			}
		}
		if(y<mymapdimension-1){
			if(thistiles[x,y+1] != "Wall"){//Checking Down
				if(turns == 0 && thistiles[x,y+1] == "Goal"){
				}
				else{
	//				Debug.Log("Cloning Down");
					Worker worker = new Worker(turns, x, y, thistiles, "Down", newgenes, newstoppedtiles, lrud, lrudpos, newpiecesused, newpiecesturned);
					workers.Add(worker);
				}
			}
		}
		if(y>0){
			if(thistiles[x,y-1] != "Wall"){//Checking Up
				if(turns == 0 && thistiles[x,y-1] == "Goal"){
					...
					Worker worker = new Worker(turns, x, y, thistiles, "Up", newgenes, newstoppedtiles,  lrud, lrudpos, newpiecesused, newpiecesturned);
					workers.Add(worker);
				}
			}	
		}
	...
	public void Anotherturn(){
		workers = new List<Worker>();																	//Clears worker class to repopulate with clones
			for(int i=0; i<lastgen.Count ; i++){ 
				Worker lastworker = lastgen[i];
															//for every one in previous gen
				...
				CheckAndCreate(lastworker.turns, lastworker.x, lastworker.y, lastworker.mytiles, lastworker.mygenes, 
					lastworker.stoppedtiles, lastworker.lrud, lastworker.lrudpos, lastworker.piecesused, lastworker.piecesturned);
				//Debug.Log(workers.Count);
...
//						Debug.Log(startx + "+" + starty);
	...
	public void PlacePiece(int degree, int x, int y, int piecenumber){
		List <string> mypiecetiles = CreateMethod.piecetiles;
		if(degree == 0){	

			if(mypiecetiles[piecenumber] == "Left" || mypiecetiles[piecenumber] == "Right" 
			|| mypiecetiles[piecenumber] == "Up" || mypiecetiles[piecenumber] == "Down"){//check if lrud
				
				newtiles[x,y] = "Wall";

				placeIcarus(0,currenttest, mypiecetiles[piecenumber]);
				// remove the lefted one from possibletiles (in next ones)

			}
			else{
//				Debug.Log("degree 0 has " + CreateMethod.piecetiles[piecenumber]);
				newtiles[x,y] = mypiecetiles[piecenumber];
		...
		if(degree == 1){	
			...
			||mypiecetiles[piecenumber] == "Up" || mypiecetiles[piecenumber] == "Down"){//check if lrud
				...
				placeIcarus(1,currenttest2, mypiecetiles[piecenumber]);
				// remove the lefted one from possibletiles (in next ones)

			}
			else{
			
				newertiles[x,y] = mypiecetiles[piecenumber];
		...
		if(degree == 2){	
			...
			|| mypiecetiles[piecenumber] == "Up" || mypiecetiles[piecenumber] == "Down"){//check if lrud
				...
				placeIcarus(2,currenttest3, mypiecetiles[piecenumber]);
				// remove the lefted one from possibletiles (in next ones)

			}
			else{
			
				newerertiles[x,y] = mypiecetiles[piecenumber];
//				Debug.Log("degree 2 has " + CreateMethod.piecetiles[piecenumber]);
	...
	public bool HasLoop(int x, int y, int piece1, int piece2, int iceplace){
		List<string> piecetiles = CreateMethod.piecetiles;
		if((piecetiles[piece1] == "Up" || piecetiles[piece1] == "UpSeed")&& 
			(piecetiles[piece2] == "Down" || piecetiles[piece2] == "DownSeed")){
		...
		if((piecetiles[piece1] == "Down" || piecetiles[piece1] == "DownSeed") && 
			(piecetiles[piece2] == "Up" || piecetiles[piece2] == "UpSeed")){
			if(x==(int)possibletiles[iceplace].x && (y <(int)possibletiles[iceplace].y)){
				return true;
			}
			return false;
		...
		if((piecetiles[piece1] == "Left" || piecetiles[piece1] == "LeftSeed") && 
			(piecetiles[piece2] == "Right" || piecetiles[piece2] == "RightSeed")){
		...
		if((piecetiles[piece1] == "Right" || piecetiles[piece1] == "RightSeed")&& 
			(piecetiles[piece2] == "Left" || piecetiles[piece2] == "LeftSeed")){
			if(y==(int)possibletiles[iceplace].y && (x <(int)possibletiles[iceplace].x)){
				return true;
			}
			return false;
		...
		switch(CreateMethod.piecetiles.Count){
			...
			case 1:
				...
				for(int i= 0; i<possibletiles.Count ; i++){ //loop through all tiles ogtiles, tries single piece in every possible ice tile
						xtocheck = (int)possibletiles[i].x;
						ytocheck = (int)possibletiles[i].y;
						InitiateNewSolution();
						UpdateMapWithPiece(xtocheck,ytocheck,0);
						PlacePiece(0,xtocheck,ytocheck,0);
...
//											Debug.Log(x3 + "" +y3);
	...
	public void CheckStoppedSeed(Solution solutiontocheck){ //for filtering out solutions where you stand on top of the seed.
		//gottago = false;
		//Debug.Log("checking stopped");
		for (int i = 0; i < CreateMethod.piecetiles.Count; i++){
			string piecetag = solutiontocheck.piecetags[i];
			List<Vector2> solpos = solutiontocheck.solutionpositions;
			if (piecetag == "WallSeed" || piecetag == "LeftSeed" || 
				piecetag == "RightSeed" || piecetag == "UpSeed" || piecetag== "DownSeed"){
				List<Vector2> stopped = solutiontocheck.stoppedtiles;
				for (int j = 0; j<stopped.Count; j++){
					if(stopped[j] == solpos[i]){
						//Debug.Log("SeedStopped");
						gottago = true;
					}
				}
			}
...
//				Debug.Log(solutions[i].myturns + "Turns with no piece" );
				CountOrStay(cursol.myturns, cursol);
			}
