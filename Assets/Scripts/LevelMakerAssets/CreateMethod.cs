...
//		Debug.Log(SolveMethod.bestsolution.piecesused + " Plus " + piecetiles.Count);
		int numberused = SolveMethod.bestsolution.piecesused;
		if(numberused > piecetiles.Count){
			Debug.Log(numberused + " Plus " + piecetiles.Count);
			return true;
		}
		else{
			return false;			
	...
	public bool DoubleStopped(){
		List<Vector2> stopped = SolveMethod.bestsolution.stoppedtiles;
		int stoppedcount = stopped.Count;
		for (int i=0; i<stoppedcount; i++){
			for(int j = i+1; j<stoppedcount; j++){
...
//		monstertile2 = monster2;
		...
		for(int i=0; i<num;i++){
			...
			Createfourbyfour();
			List<int> bestsols = SolveMethod.bestsolutions;
			int wallhugs = SolveMethod.besthaswallhug.Count;
//				Debug.Log(SolveMethod.bestsolutions.Count);
			switch(bestsols.Count){

				case 3:
					if(bestsols[0] == 0 && bestsols[1] == 0 && bestsols[2]>startingturns-1){// && SolveMethod.bestsolution.lrud >0){	
						//Debug.Log("Conditions were met ");
						if(SolveMethod.besthaswallhug.Count == 0 && !SolveMethod.gottago){ //CHECKS FOR WALLHUG IN SOLUTION
							if(!CheckWallHug() &&(DoubleHit() || DoubleStopped())){
								ConvertMapToText();
								DrawMap();
								goodmapsbuilt++;
								Debug.Log("Map numero " + (goodmapsbuilt) + " Turns " + SolveMethod.bestsolutions[2] + " " + i);
				...
				case 2:
					if(bestsols[0] == 0 && bestsols[1] > startingturns-1){ //&& SolveMethod.bestsolution.lrud >0){
						//Resources.UnloadUnusedAssets();
						
						//Debug.Log("Conditions were met");
						if(wallhugs == 0 && !SolveMethod.gottago){ //CHECKS FOR WALLHUG IN SOLUTION\
							//if(AppropriateLrud() && !CheckWallHug() && !CheckBoring() &&DoubleHit()){
							if(!CheckWallHug() && !CheckBoring()){
								ConvertMapToText();
								DrawMap();
								Resources.UnloadUnusedAssets();
								goodmapsbuilt++;
								Debug.Log("Map numero " + (goodmapsbuilt) + " Turns " + bestsols[1] + " " + i);
								PrintGenes();
	...
	public void PrintGenes(){
		string stp = "";
		for(int i=0; i<SolveMethod.bestsolution.mygenes.Count; i ++ ){
			stp = stp + "," + SolveMethod.bestsolution.mygenes[i];
			//Debug.Log(SolveMethod.bestsolution.mygenes[i]);
		}
		Debug.Log(stp);
	}
...
//				Debug.Log(SolveMethod.bestsolutions.Count);
				switch(bestsols.Count){
					case 3:
						if(bestsols[0] == 0 && bestsols[1] == 0 && bestsols[2]>startingturns-1){// && SolveMethod.bestsolution.lrud >0){	
							//Debug.Log("Conditions were met ");
							if(wallhugs == 0 && !SolveMethod.gottago){ //CHECKS FOR WALLHUG IN SOLUTION
								if(!CheckWallHug() &&(!CheckBoring() || DoubleHit() || DoubleStopped())){
	//								Debug.Log("Conditions were met without hug ");
						...
						if(bestsols[0] == 0 && bestsols[1] > startingturns-1){ //&& SolveMethod.bestsolution.lrud >0){
							...
							if(wallhugs == 0 && !SolveMethod.gottago){ //CHECKS FOR WALLHUG IN SOLUTION\
								...
								if(!CheckWallHug() && !CheckBoring()){// && (DoubleHit() || DoubleStopped())){
																//Debug.Log("Conditions were met without hug ");
									conditionsmet = true;
									Debug.Log("Map numero " + (i+1) + " Turns " + bestsols[1]);
					...
					case 4:
						...
						if(bestsols[0] == 0 && bestsols[1] == 0 && bestsols[2] == 0 && bestsols[3] > startingturns-1){
	...
	public bool CheckBoring(){
		Solution curBestSol = SolveMethod.bestsolution;
		for (int i=0; i < curBestSol.solutionpositions.Count; i++){
			string piecetag = curBestSol.piecetags[i];
			if(piecetag == "Wall" || piecetag == "WallSeed"){
				if(Vector2.Distance(curBestSol.solutionpositions[i], CreateMethod.goalpos) > 1.5){
			...
			if(piecetag == "Left" || piecetag == "Right" || 
				piecetag == "Up" || piecetag == "Down" ||
				piecetag == "LeftSeed" || piecetag == "RightSeed" || 
				piecetag == "UpSeed" || piecetag == "DownSeed"){
				
				//Debug.Log(Vector2.Distance(SolveMethod.bestsolution.solutionpositions[i], CreateMethod.goalpos));
				if(Vector2.Distance(curBestSol.solutionpositions[i], CreateMethod.goalpos) != 2){
...
//		Debug.Log(lrudpieces + " lrud" + SolveMethod.bestsolution.lrud);
	...
	public bool CheckWallHug(){
		Solution curBest = SolveMethod.bestsolution;
		string piecetag;
		for (int i=0; i < curBest.solutionpositions.Count; i++){
			piecetag = curBest.piecetags[i];
			int solx = (int)curBest.solutionpositions[i].x;
			int soly = (int)curBest.solutionpositions[i].y;
 			//Debug.Log("piece number " + i + " is " + SolveMethod.bestsolution.piecetags[i]);

			if((curBest.piecetags[i] == "Left" ||
				curBest.piecetags[i] == "LeftSeed" ) && solx-2>-1){	
				if(themap[solx-2, soly] == "Wall"){
					
					return true;
				}
				return false;
			}
			if((curBest.piecetags[i] == "Right" ||
				curBest.piecetags[i] == "RightSeed") && solx+2< totaldimensions){	
				if(themap[solx+2, soly] == "Wall"){
					return true;
				}	
				return false;			
			}		
			if((curBest.piecetags[i] == "Up" ||
				curBest.piecetags[i] == "UpSeed" )&& soly-2>-1){	
				if(themap[solx, soly-2] == "Wall"){
					return true;
				}	
				return false;
			}		
			if((curBest.piecetags[i] == "Down" ||
				curBest.piecetags[i] == "DownSeed") && soly+2<totaldimensions ){	
				if(themap[solx, soly+2] == "Wall"){
					return true;
				}	
				return false;
...
//		Debug.Log(curstring);
	...
	void Update () {
		...
		// if(!running && goodmapsbuilt < numberofwantedmaps){
		// 	running = true;
		// 	Debug.Log("Starting new batch");
		// 	Resources.UnloadUnusedAssets();
	...
	public void AssignMaxSpawners(int thedimension){
		switch(thedimension){
			...
			case 4:
				...
				lavamax = 0;//Random.Range(0,3);
				woodmax = 3;//Random.Range(0,2);
...
//					Debug.Log(thisError);
	...
	public void CreateTest(){
		...
		testmap[3,6] = "Fragile";
		testmap[5,6] = "Hole";
		SolveMethod.startx = 0;
		SolveMethod.starty = 4;
		piecetiles.Add("Wall");
		piecetiles.Add("Wall");
		DrawMap();
		...
		//PrintMap();
		DrawMap();
		Debug.Log("Map numero " + "test" +" Turns " + SolveMethod.bestsolutions[0]);
		//Debug.Log(DoubleStopped());
	...
	public void CreateTest2(){
		...
		//DrawMap();
		Debug.Log(SolveMethod.bestsolutions.Count);
		foreach(int sol in SolveMethod.bestsolutions){
			Debug.Log(sol);
		}
	}
...
//		Debug.Log(tag);
	...
	public static void ResetAll(){
		SolveMethod.solutions = new List<Solution>();
		CreateMethod.piecetiles = new List<string>();
		//ResetColor(); //notnow
		doorable = new List<Vector2>();
	}
...
//		Debug.Log(cleanice.Count + "Icetiles");
	...
	public void ExcludeAdjacent(){
		temporarilyoutice = new List<Vector2>();
...
//		Debug.Log(randomint);
		...
		int[] validchoices3 = {0,1,2,3,4,6,7,8,9,10};//,5,6,7,8,9,10};
