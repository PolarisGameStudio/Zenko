...
// 	string[] tut_button_sp = new string[]{"¡Acomoda las criaturas y ayúdame a llegar a la flor roja!", "Una vez que me muevo, ¡no podrás cambiar las piezas de lugar!", "Si no logras resolver el nivel, pide una pista después de reiniciarlo"};
//     	string[] tut_1_sp = new string[]{"¡Hola!, mi nombre es Zenko", "Ayúdame a llegar a la flor roja deslizando tu dedo por la pantalla"};
// 	string[] tut_2_sp = new string[]{"Esa pieza se llama Pedro", "Puedes moverla por el tablero para poder utilizarlo como barrera", "¡Acomódalo antes de moverme hacia la flor roja!"};
// 	string[] tut_3_sp = new string[]{"¡No me gustaría terminar en ese agujero!"};
// 	string[] tut_6_sp = new string[]{"Asegúrate de que ambos Pedro estén en el lugar correcto ANTES de moverme hacia la flor"};
// 	string[] tut_9_sp = new string[]{"Solo obtendrás las 3 estrellas si solucionas el tablero en la menor cantidad de movimientos posibles.", "Los puntos al lado de la barra representan la menor cantidad de movimientos posibles"};
// 	string[] tut_12_sp = new string[]{"¡Esa pequeña flor amarilla está creciendo!", "Eso significa que la primavera está cerca", "¡No pongas nada encima, hay que conservarlas!"};
//     	string[] tut_41_sp = new string[]{"Esos pájaros se llaman Icarus", "Envían una fuerte corriente de aire frente a ellos", "Si paso frente a ellos, ¡cambiarán mi dirección!"};
//     	string[] tut_50_sp = new string[]{"Eso es piso frágil", "Se destruye rápidamente... pero yo soy más rápido"};
// 	string[] tut_52_sp = new string[]{"Recuerda no detenerme sobre el piso frágil, o me caeré", "Puedo pasar sobre ellas, pero no soportan mi peso al detenerme"};				
//     	string[] tut_55_sp = new string[]{"Tengo la certeza de que el aire de Icarus es más rápido que el suelo frágil"};
//     	string[] tut_68_sp = new string[]{"¡No siempre necesito aire para llegar a la flor!"};
//     	string[] tut_81_sp = new string[]{"¡Esa bolsa con el círculo se transforma en Pedro al pasar sobre ella!"};
//     	string[] tut_121_sp = new string[]{"¡Dentro de esa bolsa con la flecha está Icarus dormido!, "Lo despertaré al pasar encima"};
//     	string[] tut_122_sp = new string[]{"La dirección de la flecha en la bolsa, ¡es la misma dirección en la Icarus soplará al despertar!"};
// 	string[] tut_160_sp = new string[]{"Aquí termina nuestra aventura por ahora", "¡Recuerda regresar pronto para jugar en nuevos niveles y conocer nuevos amigos!"};
	
//     //Choose one of the dialogs to happen after 30s of innactivity

//     string[] random1_sp = new string[] {"¡Concéntrate!¡Tú puedes!"};
//     string[] random2_sp = new string[] {"Si llegas a tener problemas acomodando las piezas, ¡presiona el botón con el foco!"};
//     string[] random3_sp = new string[] {"Cuando no encuentres la respuesta, ¡vuelve a empezar!"};
//     //Choose one of the dialogs to happen after 5 consecutive deaths
//     string[] random4_sp = new string[] {"Puedes reiniciar el nivel y pedir una pista en cuanto regrese a mi posición inicial"};
//     string[] random5_sp = new string[] {"¡Estuvimos a punto de lograrlo! ¡Sigue intentando!"};
//     string[] random6_sp = new string[] {"¡Esta vez sí lo vamos a lograr!"};


// 	string[] music_sp = new string[] {"Música"};
