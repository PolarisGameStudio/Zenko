...
public class Worker{
	...
	public List<int> piecesturned;
	public int currentpiecesturned;
	string [,] ogtiles;
	//public <Vector2> pieces

	public Worker(int newturns, int newx, int newy,string[,] newtiles, 
		string newdirection, List<string> newgenes, List<Vector2> newstopped, int newlrud, List<Vector2> newlrudpos, int newpiecesused, List<int> newpiecesturned){
		...
		lrud = newlrud;
		lrudpos = newlrudpos;
		piecesturned = new List<int>(newpiecesturned);
		currentpiecesturned = piecesturned[piecesturned.Count-1];
		ogtiles = SolveMethod.ogtiles;
...
//			Debug.Log(newx + "" + newy);
		...
		for(int i=0; i<stoppedtiles.Count; i++){
			//Vector2 stopped = stoppedtiles[i];
			if(stoppedtiles[i].x == x && stoppedtiles[i].y == y){ //if repeated stoppedtile
				//Debug.Log("Repeated at " + x + "" + y);
				if(piecesturned[i] == currentpiecesturned){
					done = true;
					break;					
				}
		...
		if(done != true){
			...
			piecesturned.Add(currentpiecesturned);
			//stoppedtiles.Add(laststopped);
		}
	...
	public bool CheckRepetition(){
		for(int i=0; i<stoppedtiles.Count; i++){
			if(stoppedtiles[i].x == x && stoppedtiles[i].y == y){ //if repeated stoppedtile
				if(currentpiecesturned == piecesturned[i]){
...
//				Debug.Log("best turns is" + SolveMethod.bestturns);
				...
				//Debug.Log("numofturnsgoingintogoal" + turns);
				//Debug.Log(SolveMethod.bestturns + " is bestturns");
				if(turns<=SolveMethod.bestturns || SolveMethod.bestturns == 0){
					//Debug.Log("Added solution with" + turns);
...
//				Debug.Log(CreateMethod.goalpos);
				//Debug.Log(CreateMethod.startingturns + "Are to beat");
...
//			Debug.Log(tilex + "" + tiley);
	...
	public void AssignPieces(){
		//string[,] ogtiles = SolveMethod.ogtiles;
		for(int i=0; i<8; i++){
			for(int j=0; j<8; j++){
				if(mytiles[i,j]!=ogtiles[i,j]){
